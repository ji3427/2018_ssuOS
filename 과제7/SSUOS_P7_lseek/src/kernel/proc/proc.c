...
void lseek_proc(void *aux , void *filename)
{
	...
	if(strcmp(aux,"-e") == 0){
		printk("e option\n");
		printk("----------e option but move file start front----------\n");
		printk("may be start : %d\n",lseek(fd,-7,SEEK_SET,aux));
		printk("----------e option at SEEK_CUR----------\n");
		printk("cur pos : %d\n",lseek(fd,4,SEEK_SET,NULL));
		printk("lseek(fd,30,SEEK_CUR,\"e\")\n");
		printk("new file size %d\n",lseek(fd,30,SEEK_CUR,aux));
		lseek(fd,0,SEEK_SET,NULL);
		memset(buf,0,BUFSIZ);
		length = read(fd , buf, BUFSIZ);
		printk("full sentence : %s\n",buf);
		printk("----------extend space write----------\n");
		lseek(fd,-9,SEEK_END,NULL);
		printk("lseek(fd,-9,SEEK_END,NULL) & write(fd,\"20142447\",8)\n");
		write(fd,"20142447",8);
		lseek(fd,0,SEEK_SET,NULL);
		memset(buf,0,BUFSIZ);
		read(fd, buf, BUFSIZ);
		printk("full sentence : %s\n",buf);
		printk("----------e option at SEEK_END----------\n");
		printk("lseek(fd,10,SEEK_END,\"e\")\n");
		printk("new file size %d\n",lseek(fd,10,SEEK_END,aux));
		lseek(fd,0,SEEK_SET,NULL);
		memset(buf,0,BUFSIZ);
		read(fd , buf, BUFSIZ);
		printk("full sentence : %s\n",buf);
		...
		printk("a option\n");
		printk("-------------a option at SEEK_CUR----------------------\n");
		printk("cur pos : %d\n",lseek(fd,4,SEEK_SET,NULL));
		printk("lseek(fd,3,SEEK_CUR,\"a\")\n");
		printk("after cur pos : %d\n",lseek(fd,3,SEEK_CUR,aux));
		lseek(fd,0,SEEK_SET,NULL);
		memset(buf,0,BUFSIZ);
		read(fd, buf, BUFSIZ);
		printk("full sentence : %s\n",buf);
		printk("-------------a option at SEEK_SET----------------------\n");
		printk("lseek(fd,3,SEEK_SET,\"a\")\n");
		printk("after cur pos : %d\n",lseek(fd,3,SEEK_SET,aux));
		lseek(fd,0,SEEK_SET,NULL);
		memset(buf,0,BUFSIZ);
		read(fd, buf, BUFSIZ);
		printk("full sentence : %s\n",buf);
		printk("-------------a option at SEEK_END----------------------\n");
		printk("lseek(fd,3,SEEK_END,\"a\")\n");
		printk("after cur pos : %d\n",lseek(fd,3,SEEK_END,aux));
		lseek(fd,0,SEEK_SET,NULL);
		memset(buf,0,BUFSIZ);
		read(fd, buf, BUFSIZ);
		printk("full sentence : %s\n",buf);
		...
		printk("re option\n");
		printk("----------re option but file end back----------\n");
		printk("may be end : %d\n",lseek(fd,7,SEEK_END,aux));
		printk("----------re option at SEEK_CUR----------\n");
		printk("cur pos : %d\n",lseek(fd,2,SEEK_SET,NULL));
		printk("lseek(fd,-15,SEEK_CUR,\"re\")\n");
		lseek(fd,-15,SEEK_CUR,aux);
		printk("new file size %d\n",lseek(fd,0,SEEK_END,NULL));
		lseek(fd,0,SEEK_SET,NULL);
		memset(buf,0,BUFSIZ);
		read(fd , buf, BUFSIZ);
		printk("full sentence : %s\n",buf);
		printk("----------extend space write----------\n");
		lseek(fd,3,SEEK_SET,NULL);
		printk("lseek(fd,3,SEEK_SET,NULL); & write(fd,\"abcdef\",6)\n");
		write(fd,"abcdef",6);
		lseek(fd,0,SEEK_SET,NULL);
		memset(buf,0,BUFSIZ);
		read(fd, buf, BUFSIZ);
		printk("full sentence : %s\n",buf);
		printk("----------re option at SEEK_SET----------\n");
		printk("lseek(fd,-10,SEEK_SET,\"re\")\n");
		printk("lseek result %d\n",lseek(fd,-10,SEEK_SET,aux));
		printk("new file size %d\n",lseek(fd,0,SEEK_END,NULL));
		lseek(fd,0,SEEK_SET,NULL);
		memset(buf,0,BUFSIZ);
		read(fd , buf, BUFSIZ);
		printk("full sentence : %s\n",buf);
		printk("----------re option over buffer----------\n");
		printk("cur size : %d\n",lseek(fd,0,SEEK_END,NULL));	
		printk("lseek(fd,-129,SEEK_SET,\"re\")\n");
		printk("lseek result %d\n",lseek(fd,-129,SEEK_SET,aux));
		length = lseek(fd,0,SEEK_END,NULL);
		printk("new file size %d\n",length);
		lseek(fd,0,SEEK_SET,NULL);
		memset(buf,0,BUFSIZ);
		read(fd , buf, BUFSIZ-1);
		printk("full sentence : %s",buf);
		memset(buf,0,BUFSIZ);
		read(fd , buf, length - BUFSIZ+1);
		printk("%s\n",buf);
	}
	else if(strcmp(aux,"-c") == 0){
		printk("c option\n");
		printk("cur file size %d\n",lseek(fd,0,SEEK_END,NULL));
		printk("lseek(fd,-10,SEEK_SET,\"c\")\n");
		printk("cur pos : %d\n",lseek(fd,-10,SEEK_SET,aux));
		printk("lseek(fd,20,SEEK_CUR,\"c\")\n");
		printk("cur pos : %d\n",lseek(fd,20,SEEK_CUR,aux));
		printk("lseek(fd,20,SEEK_END,\"c\")\n");
		printk("cur pos : %d\n",lseek(fd,-30,SEEK_CUR,aux));
