...
void init_kbd(void)
{
	cur_foreground_process->kbd_buffer=get_kbd_buffer(); //foreground process에 버퍼할당
	KStat.ShiftFlag = 0;
	KStat.CapslockFlag = 0;
	KStat.NumlockFLag = 0;
	KStat.CtrlFlag = 0; //ctrl flag 초기화
...
void UpdateKeyStat(BYTE Scancode)
{
	if(Scancode & 0x80)
	{
		...
		else if( Scancode == 0x9D){ //ctrl 땟을경우
	...
	else
	{
		...
		else if(Scancode == 0x1D){ //컨트롤을 누른 경우
			KStat.CtrlFlag = TRUE;
		}
	}
...
void kbd_handler(struct intr_frame *iframe)
{
	...
	{
		if(KStat.CtrlFlag == TRUE){ //ctrl이 눌린경우
			switch(asciicode){
				case 'l':
					clearScreen();
					break;
		...
		else if( (!isFull() && asciicode != 0) && KStat.CtrlFlag == FALSE) //ctrl과 같이 문자열이 입력된경우 출력이 안되게 하기위해 CtrlFlag == FALSE 추가
		{
