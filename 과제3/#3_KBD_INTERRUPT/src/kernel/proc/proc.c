...
static void background_test(void *);
...
void login_prompt(void * aux)
{
	char id[BUFSIZ];
	char password[BUFSIZ];
	struct process *temp = cur_foreground_process;
	while(1)
	{
		printk("\nid : ");
		while(getkbd(id,BUFSIZ) == TRUE)
		{
			while(temp!=cur_foreground_process); //현재 foreground process가 변경된경우 프로세스를 중지
			;
		}
...
void background_test(void * aux) //background 프로세스를 test하기 위한 함수
{
	int j =0;
	for(int i = 0; i<10;i++){
		for(int j = 0 ; j < 5000000 ; j++);
		printk("background test\n");
	}
	loop_proc(aux);
...
void init_proc()
{
	...
	cur_foreground_process = cur_process; //foreground test 초기화
	cur_process->time_used = 0;
	cur_process->time_sched= 0;
	...
	list_push_back(&f_list, &cur_process->elem_foreground); //f_list에 추가
}
...
pid_t proc_create(proc_func func, struct proc_option *opt, void* aux)
{
	...
	*(--top) = 6; //edi
	...
	if(opt->foreground == TRUE){
		...
		p->console->Glob_x = 0;
		p->console->Glob_y = 2;
		p->console->buf_w = p->console->buf_s;
		p->console->buf_p = p->console->buf_s;
		p->console->a_s = TRUE;
		p->console->sum_y = 0;
		cur_console = p->console;//현재 콘솔을 새로운 프로세스의 콘솔로 변경
		cur_foreground_process = p; //foreground process 변경
		init_kbd(); //새로운 입력버퍼 할당
		list_push_back(&f_list,&p->elem_foreground); //f_list에 추가
	}
	else{
		p->console = cur_console; //백그라운드의 프로세스의 경우 부모의 출력버퍼를 사용
	}
...
void create_background(void* aux)
{
	struct proc_option proc_opt = {0, FALSE};
	proc_create(background_test,&proc_opt,NULL);
}
...
void shell_proc(void* aux)
{
	CmdList cmdlist[] = {
		...
		{"create_background",0,create_background}
	};
#define CMDNUM 5
#define TOKNUM 10
	char buf[BUFSIZ];
