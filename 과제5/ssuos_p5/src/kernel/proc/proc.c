...
pid_t proc_create(proc_func func, struct proc_option *opt, void* aux)
{
	...
	p->simple_lock = 0;
	p->child_pid = -1;
	pid_t temp = cur_process -> pid; //임시변수에 현재의 pid 값 저장
	cur_process -> pid = p->pid; //현재 프로세스의 자식의 pid 값 저장
	child_stack_reset(temp); // 자식 프로세스의 스택 초기화
    int *top = (int*)palloc_get_multiple(STACK__, 2);
	int stack = (int)(top-1);
	cur_process->pid = temp; //현재 프로세스의 pid 복구
