...
int init_intr(void)
{
	...
	printk("%s", "Keyboard Handler Registration\n");

	reg_handler(0x30, syscall_handler);
	printk("%s", "System Call Handler Registration\n");
...
{
	ticks++;	
	if ((cur_process -> pid != 0) && (!scheduling) && (cur_process->state == PROC_RUN)) {//프로세스의 현재 상태가 run 상태이면서 스케쥴링 변수가 0인경우 동작
		cur_process->time_used++;
		cur_process->time_slice++;
		if(cur_process->que_level == 1){ //현재 프로세스의 queue_level이 1인 경우 
			if(cur_process->time_slice >= LV0_TIMER){ //현재 프로세스의 time_slice값이 LV0_TIMER(=40)보다 커진 경우 스케쥴링 해줌
				do_sched_on_return();
			}
		}
		if(cur_process->que_level == 2){
			if(cur_process->time_slice >= LV1_TIMER){//현재 프로세스의 time_slice값이 LV1_TIMER(=80)보다 커진 경우 스케쥴링 해줌
				do_sched_on_return();
			}
		}	
